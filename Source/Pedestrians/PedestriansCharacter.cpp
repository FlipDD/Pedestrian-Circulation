...
void APedestriansCharacter::BeginPlay()
{
	...
	TSubclassOf<APedestriansCharacter> PedestrianClass = APedestriansCharacter::StaticClass();
	...
	for (int i = 0; i < AAgents.Num(); i++)
		Agents.Add(Cast<APedestriansCharacter>(AAgents[i]));
...
void APedestriansCharacter::Tick(float DeltaTime)
{
	...
	while (i < Walls.Num())
	{
		...
		{
			dist = FVector::Dist(MyLoc, WLoc); // UKismetMathLibrary::Sqrt((MyLoc.X - WLoc.X) * (MyLoc.X - WLoc.X));
			nX = MyLoc.X - WLoc.X;
			nX = nX / dist;
			...
			MyLoc.X < (WLoc.X + WScale.X / 2))
		{
			...
			dist = FVector::Dist(MyLoc, WLoc);//UKismetMathLibrary::Sqrt((MyLoc.Y - WLoc.Y) * (MyLoc.Y - WLoc.Y));
			// Get vector to wall
			nY = MyLoc.Y - WLoc.Y;
			// Normalize vector
			nY = nY / dist;
...
//	DistToTarget = UKismetMathLibrary::Sqrt(MyLoc.X - TargetLoc.X)*(MyLoc.X - TargetLoc.X) + (MyLoc.Y - TargetLoc.Y)*(MyLoc.Y - TargetLoc.Y);
//
//	if (DistToTarget <= 20 * radius)
//	{
//		if (targNr < Targets.Num() - 1)
